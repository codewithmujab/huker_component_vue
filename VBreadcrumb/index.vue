    ...
    <div>
        <ul class="inline-flex flex-wrap text-sm font-medium sm:mb-6 mb-4">
            <li class="after:content-['/'] last:after:hidden after:text-slate-400 after:px-2"
                ...
                <a :id="route.active ? 'active' : 'passive'" class="hover:text-primary cursor-pointer"
                    :class="route.active ? 'text-primary' : 'text-slate-500'" @click="redirectToPage(route, index)">{{
                        route.name }}</a>
            </li>
        </ul>
    </div>
