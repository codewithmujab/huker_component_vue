...
<template>
    <div class="relative inline-flex">
        <button ref="trigger"
            ...
            <svg class="w-4 h-4 fill-current" :class="filterExists ? 'fill-primary' : 'fill-slate-500'"
                viewBox="0 0 16 16">
                <path
        ...
        <transition enter-active-class="transition ease-out duration-200 transform"
            ...
            <div v-show="dropdownOpen"
                class="origin-top-right z-[29] absolute top-full min-w-56 bg-white border border-slate-200 pt-1.5 rounded shadow-lg overflow-visible mt-2 w-max"
                ...
                <div ref="dropdown">
                    ...
                    <div class="py-2 px-3 border-t border-slate-200 bg-slate-50">
                        <ul class="flex items-center justify-end gap-x-3">
                            ...
                            <VButton label="Apply" type="primary" @click="dropdownOpen = false; $emit('apply');"
                                size="small" />
                        </ul>
                    </div>
                </div>
