...
import { any, bool, oneOf, string } from 'vue-types';
...
<template>
    <div>
        <div class="mb-1" :class="{ 'flex items-center my-auto': tooltip, 'block': !tooltip }" v-if="label">
            <label class="block text-sm font-medium text-slate-600" :for="placeholder" v-if="label">{{ label }} <span
        ...
        <div class="relative">
            ...
            <input :id="placeholder" :class="[{
                ...
                :type="type" :placeholder="placeholder" :autocomplete="'new-' + type" :disabled="disabled"
                @input="updateValue" :value="modelValue" />
        </div>
