...
import { ref } from "vue";
import { any, bool, object, oneOf, string } from 'vue-types';
...
<template>
    <div>
        <label class="block text-sm font-medium text-slate-600 mb-1" :for="placeholder" v-if="label">{{ label }} <span
                class="text-rose-500" v-if="required">*</span></label>

        <!-- Select Type -->
        <Multiselect v-if="type === 'select'" v-model="value" v-bind="{ value: value, options: options }" :options="options"
            :searchable="true" ref="multiselect" :canClear="clearable" :canDeselect="clearable" :placeholder="placeholder"
        ...
        <!-- Multiple Type -->
        <Multiselect v-if="type === 'multiple'" v-model="value" :options="options" ref="multiselect" mode="multiple"
            :placeholder="placeholder" :close-on-select="false" :disabled="disabled" :class="{
                'medium': size === 'md',
                'danger': errorMessage
            }" />
        ...
        <Multiselect v-if="type === 'tags'" v-model="value" :disabled="disabled" :class="{
            'medium': size === 'md',
            'danger': errorMessage
        }" mode="tags" :placeholder="placeholder" :close-on-select="true" :searchable="true" :create-option="false"
            :options="options" :infinite="true" :limit="3" ref="multiselect" />

        <!-- Async Tags Type -->
        <Multiselect v-if="type === 'async-tags'" v-model="value" mode="tags" :placeholder="placeholder" :class="{
            'medium': size === 'md',
            'danger': errorMessage
        }" :close-on-select="true" :filter-results="false" :min-chars="1" :resolve-on-load="false"
