...
const props = defineProps({
    type: oneOf(['warning', 'error', 'success', 'primary']).def('primary'),
    open: bool().def(false),
})
    ...
    <div v-show="open">
        <div class="inline-flex min-w-80 px-4 py-2 rounded-sm text-sm text-white" :class="typeColor(type)">
            <div class="flex w-full justify-between items-start">
                <div class="flex">
                    <svg v-if="type === 'warning'" class="w-4 h-4 shrink-0 fill-current opacity-80 mt-[3px] mr-3"
                        viewBox="0 0 16 16">
                        <path
                            d="M8 0C3.6 0 0 3.6 0 8s3.6 8 8 8 8-3.6 8-8-3.6-8-8-8zm0 12c-.6 0-1-.4-1-1s.4-1 1-1 1 .4 1 1-.4 1-1 1zm1-3H7V4h2v5z" />
                    </svg>
                    <svg v-else-if="type === 'error'" class="w-4 h-4 shrink-0 fill-current opacity-80 mt-[3px] mr-3"
                        viewBox="0 0 16 16">
                        <path
                    ...
                    <svg v-else-if="type === 'success'" class="w-4 h-4 shrink-0 fill-current opacity-80 mt-[3px] mr-3"
                        viewBox="0 0 16 16">
                        <path
